...
const DashboardPage = (() => {
  ...
  function _guideHTML() {
    const sections = [
      ...
      {
        icon: '🔁', title: '固定收支（每日自動套用 + 手動快速記帳）',
        desc: '每天第一次開啟 App 時，系統會自動套用當月應入帳的固定收支。也可到「記帳」→「🔁 固定」Tab 手動點擊確認，金額可修改後送出——若當月同類別已有記帳，會更新金額而非重複新增。若月底仍有未入帳項目，系統會在每月最後一天 23:00 自動補齊。同月只會記一次，不會重複。'
      },
      {
        icon: '⚙️', title: '固定收支管理',
      ...
      {
        icon: '📋', title: '歷史記錄',
        desc: '點底部「記錄」查看每月記帳明細，可依類型與類別篩選，左右箭頭切換月份。每日標題會同時顯示當日支出與收入合計，方便掌握每天的金流。點擊單筆記帳可查看詳情（含記帳對象）或刪除。'
      },
      {
        icon: '📊', title: '統計分析',
