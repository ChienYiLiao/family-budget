...
const AvatarCropper = (() => {
  let _cropper = null;
  let _onDone = null;
  let _overlayId = 'avatar-cropper-overlay';
  ...
  /**
   * 開啟裁切器
   * @param {string} src - 圖片 data URL
   * @param {Function} onDone - callback({ base64, mimeType, dataUrl })
   */
  function open(src, onDone) {
    ...
    setTimeout(() => {
      if (typeof Cropper !== 'undefined') {
        ...
        // Cropper.js 未載入時，直接取整張圖
        Utils.compressImage(
          _dataUrlToFile(src, 'avatar.jpg'),
          300, 0.9
        ).then(result => {
          _destroy();
          Modal.hide(_overlayId);
